...
export class Idle implements ng.IServiceProvider, IIdleProvider {
  ...
  private id: number;
  private idleCounter: ng.IPromise<any>;
  ...
  $get(
    ...
    $keepalive: IKeepaliveService,
    $idleLocalStorage: IIdleStorage
  ): IIdleService {
  ...
  private toggleState(): void {
    ...
    if (this.state.idling) {
      ...
      if (this.options.timeout) {
        ...
        this.countdown();
        this.state.timeout = this.interval(
  ...
  private watch(noExpiryUpdate?: boolean): void {
    ...
    this.idleCounter = this.interval(() => this.state.userIdleTime++, 1000);
    this.state.idle = this.interval(
      this.toggleState.bind(this),
      this.options.idle * 1000,
      0,
      false
    );
  }
  ...
  /**
   * Stops all the intervals running.
   */
